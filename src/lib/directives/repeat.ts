...
export type RepeatPreRenderer = (renderInfo: RenderInfo) => AsyncIterableIterator<string>;
...
export const repeat = <T>(items: Iterable<T>,
   ...
   */
  const doRepeat = async function* (renderInfo: RenderInfo): AsyncIterableIterator<string> {
    let i = 0;
