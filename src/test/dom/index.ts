...
import Window from 'domino/lib/Window.js';
...
const runSSRMmodule = async (path: string) => {
  ...
  const document = domino.createDocument('', true);
  const window = new Window(document);
  const context = vm.createContext(window);
  const module = await loadModule(path, import.meta.url);
...
const server = http.createServer(async (_request, response) => {
  const result = await runSSRMmodule('./module.js');
  response.end(result);
});
const port = 8080;
server.listen(port, () => {
